...
export const renderFile = (file: FileData) => {
    if (file.type === 'image' || file.type === "key") {
        return (
            <ImageTaskComponent file={file} />
        );
    } else if (file.type === 'audio') {
        return (
            <AudioComponent audioFile={file} />
        );
    } else {
        return <Text
          //  key={file.id}
        >Неизвестный тип файла</Text>;
    }
};
...
export const convertJsonToHtml = (json: string) => {
    try {
        ...
        const htmlContent = parsedJson.content.map((paragraph: any) => {
            ...
            const paragraphHtml = paragraph.content.map((text: any) => {
                ...
                const textStyles = (text.marks || []).map((mark: any) => {
