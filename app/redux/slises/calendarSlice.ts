...
>(
    ...
    async ({ start, end, type = "my", filters }, { rejectWithValue }) => {
        try {
...
export const fetchCalendarBySeminarian = createAsyncThunk<
    ...
    { rejectValue: string }
>(
    'timetable/fetchTimetableBySeminarian',
    async ({ date, speaker_id }, { rejectWithValue }) => {
        try {
...
const calendarSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchCalendarBySeminarian.fulfilled, (state, action: PayloadAction<TimetableEvent[]>) => {
                state.status = 'succeeded';
                state.data = action.payload;
            })
