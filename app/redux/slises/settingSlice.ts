...
// УДАЛЯЕМ интерфейс Platform и всё связанное с платформами
...
// Запрос за настройками (оставляем)
...
export const registerPushToken = createAsyncThunk<
  ...
  { rejectValue: string }
>('userDevice/registerPushToken', async (params, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || 'Не удалось зарегистрировать токен')
  }
})
...
const initialState: SettingsState = {
  ...
  pushTokenData: null,
  status: 'idle',
  error: null, // УДАЛЯЕМ platformStatus
}
...
const settingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // УДАЛЯЕМ все cases для fetchPlatforms
      .addCase(registerPushToken.pending, (state) => {
        state.status = 'loading' // Меняем на status вместо platformStatus
      })
