...
export const ChatScreen = ({ navigation }: any) => {
  ...
  const [modalAction, setModalAction] = useState<
    'delete' | 'fix' | 'unFix' | 'mute' | 'unMute' | null
  >(null)
  const settings = useAppSelector((state) => state.settings.data)
  const isChildrenCanStartChat =
    settings?.find((el) => el.key === 'isChildrenCanStartChat')?.value === '1'
  ...
  const unreadCountsByChatId: Record<number, number> = (notifications as Notification[]).reduce<
        ...
        <>
          <View style={styles.container}>
            <View style={styles.filterContainer}>
              ...
              >
                <Text style={[styles.filterText, filter === 'personal' && styles.activeFilterText]}>
                  Личные
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                return (
                  ...
                  >
                    <View style={styles.chatItem}>
                      {item.isFixed && (
                        <IconButton icon="auto-fix" size={20} iconColor={Colors.colorGreen} />
                      )}
                      {item.avatar ? (
                          ...
                          >
                            ...
                            <Menu.Item
                              onPress={() => {
                                ...
                                setModalTitle(
                                  item.isFixed
                                    ? 'Вы хотите открепить данный чат?'
                                    : 'Вы хотите закрепить данный чат?'
                                )
                                setChatId(chatId)
                            ...
                            <Menu.Item
                              onPress={() => {
                                ...
                                setChatId(chatId)
                                setModalTitle(
                                  item.muted ? 'Включить уведомления?' : 'Выключить уведомления?'
                                )
